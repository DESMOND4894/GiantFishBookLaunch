...
import { generateUniqueCouponCode, normalizeAmazonOrderNumber } from "@/lib/coupon";
...
const REVIEW_PROOF_TYPES: Record<string, string> = {
  png: "image/png",
  jpg: "image/jpeg",
  jpeg: "image/jpeg",
  webp: "image/webp",
...
export async function joinLaunchTeam(
  ...
  if ((count ?? 0) >= LAUNCH_TEAM_CAP) {
    return { ok: false, message: "The launch team is full! Thank you for your interest. Stay tuned for the book launch on May 4th." };
  }
...
export async function submitReviewLink(
  ...
  const email = getValue(formData, "email").trim().toLowerCase();
  const reviewProof = formData.get("review_proof");

  if (!email || !(reviewProof instanceof File) || !reviewProof.size) {
    return { ok: false, message: "Please enter your email and upload a screenshot of your posted Amazon review." };
  }
  ...
  if (reviewProof.size > 10 * 1024 * 1024) {
    return { ok: false, message: "Screenshot must be under 10MB." };
  }

  const fileExt = (reviewProof.name.split(".").pop() || "jpg").toLowerCase();
  const contentType = reviewProof.type || REVIEW_PROOF_TYPES[fileExt] || "";

  if (!REVIEW_PROOF_TYPES[fileExt] || !Object.values(REVIEW_PROOF_TYPES).includes(contentType)) {
    return { ok: false, message: "Please upload a PNG, JPG, WEBP, or HEIC screenshot." };
  }

  const path = `review-proofs/${slugify(member.full_name || email)}-${Date.now()}.${fileExt}`;
  const bytes = Buffer.from(await reviewProof.arrayBuffer());
  ...
  if (upload.error) {
    return { ok: false, message: "We couldn't upload that screenshot. Please try again." };
  }
  ...
  const trackerPayload = {
    ...
    review_link: path,
    posted_at: now,
    notes: "Submitted through the public launch party confirmation form.",
  };
  const { data: existingReview } = await client
    .from("reviews")
    .select("id")
    .eq("launch_team_member_id", member.id)
    .order("created_at", { ascending: false })
    .limit(1)
    .maybeSingle();
  const trackerWrite = existingReview?.id
    ? await client.from("reviews").update(trackerPayload).eq("id", existingReview.id)
    : await client.from("reviews").insert({
        launch_team_member_id: member.id,
        ...trackerPayload,
      });

  if (trackerWrite.error) {
    console.error("submitReviewLink tracker write failed:", trackerWrite.error);
  }
  ...
  return { ok: true, message: "Thank you! Your review has been recorded. One more step — head to the RSVP page to lock in your seat for the Friday launch party trip so we know to expect you!" };
}
...
export async function submitRsvp(
  ...
  const fullName = getValue(formData, "full_name").trim();
  const email = getValue(formData, "email").trim().toLowerCase();
  const partySize = parsePartySize(getValue(formData, "party_size"));
  ...
  // Dedup by email so repeat submissions update the headcount instead of doubling it.
  if (email) {
    const { data: existing } = await client
      .from("launch_party_rsvps")
      .select("id")
      .ilike("email", email)
      .limit(1)
      .maybeSingle();
    ...
    if (existing) {
      const { error } = await client.from("launch_party_rsvps").update({
        full_name: fullName,
      ...
      await createActivity(`RSVP updated: ${fullName} (party of ${partySize})`, "launch_party_rsvps", existing.id, "note");
      revalidatePath("/rsvp");
      revalidatePath("/admin/rsvps");
      revalidatePath("/dashboard");
      return { ok: true, message: `Got it, ${fullName} — your RSVP is updated for a party of ${partySize}. See you Friday!` };
    }
  }

  const { data, error } = await client.from("launch_party_rsvps").insert({
    full_name: fullName,
    email: email || null,
    party_size: partySize,
    source: "self",
  ...
  await createActivity(`RSVP received: ${fullName} (party of ${partySize})`, "launch_party_rsvps", data?.id ?? null, "note");
  revalidatePath("/rsvp");
  revalidatePath("/admin/rsvps");
  revalidatePath("/dashboard");

  return { ok: true, message: `You're on the list, ${fullName}! We've got you down for a party of ${partySize}. See you Friday on the Celtic Quest!` };
}

export async function createRsvpManual(formData: FormData) {
  if (!hasSupabaseEnv()) return;
  const client = getSupabaseAdminClient();
  if (!client) return;
  ...
  const { data } = await client.from("launch_party_rsvps").insert({
    full_name: fullName,
    email: getValue(formData, "email").trim().toLowerCase() || null,
    party_size: partySize,
    source: "manual",
    notes: getValue(formData, "notes") || null,
  }).select("id").single();

  await createActivity(`RSVP added manually: ${fullName} (party of ${partySize})`, "launch_party_rsvps", data?.id ?? null, "note");
  revalidatePath("/admin/rsvps");
  revalidatePath("/dashboard");
  redirect("/admin/rsvps?saved=rsvp-added");
}
...
export async function deleteRsvp(formData: FormData) {
  ...
  await client.from("launch_party_rsvps").delete().eq("id", id);
  await createActivity("RSVP removed", "launch_party_rsvps", id, "note");
  revalidatePath("/admin/rsvps");
  revalidatePath("/dashboard");
  redirect("/admin/rsvps?saved=rsvp-removed");
}
...
export async function submitCouponClaim(
  ...
  const email = getValue(formData, "email").trim().toLowerCase();
  const rawOrderNumber = getValue(formData, "amazon_order_number");
  const orderNumber = normalizeAmazonOrderNumber(rawOrderNumber);

  if (!firstName || !lastName || !email || !orderNumber) {
    return { ok: false, message: "Please enter your name, email, and a valid Amazon order number." };
  }
  ...
  if (existingOrder) {
    return { ok: false, message: "It looks like this Amazon order number has already been used for a coupon claim." };
  }
  ...
  } else if (!cqResult.ok && claim?.id) {
    await client.from("coupon_claims").update({
      admin_notes: `CQ sync failed: ${cqResult.error || "Unknown error"}`,
    }).eq("id", claim.id);
  }
...
export async function sendCoupon(
  ...
  if (!claim.cq_coupon_id) {
    const syncResult = await syncCouponToCq({
    ...
    if (!syncResult.ok || !syncResult.cqCouponId) {
      await client.from("coupon_claims").update({
        admin_notes: `CQ sync failed: ${syncResult.error || "No CQ coupon id returned"}`,
      }).eq("id", claimId);
      return { ok: false, message: "Coupon was not sent because it is not synced to Celtic Quest yet. Fix CQ sync and try again." };
    }

    await client.from("coupon_claims").update({ cq_coupon_id: syncResult.cqCouponId }).eq("id", claimId);
  }
  ...
  if (!emailResult.ok) {
    await client.from("coupon_claims").update({
      admin_notes: `Email send failed: ${emailResult.error || "Unknown error"}`,
    }).eq("id", claimId);
